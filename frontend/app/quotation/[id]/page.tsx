    ...
    type QuotationItem = { 
        ...
        price: number; 
        unit_price: number; 
        unit_cost?: number; // New field for advanced pricing
        reasoning: string; 
        image_url?: string; 
    ...
    export default function QuotationEditor() {
        ...
        const [globalMargin, setGlobalMargin] = useState<number>(30); // Default 30% margin
        ...
        async function loadData() {
            try {
                ...
                const mappedItems = (q.content?.matches || []).map((m: any) => {
                    ...
                    // If cost is missing, estimate it as 60% of list price (standard industry practice)
                    const estimatedCost = m.unit_cost || (unitPrice * 0.6); 
                    return {
                        ...m,
            ...
            try { 
                ...
                const newMappedItems = (res.matches || []).map((m: any) => {
                    ...
                    return { 
                        ...
                        quantity: m.quantity || 1, 
                        unit_price: uPrice, 
                        unit_cost: uPrice * 0.6, // Default cost estimate
                        image_url: m.image_url || "", 
                        alternatives: m.alternatives || [] 
                    };
                }); 
        ...
        const handleSelectAlternative = (matchIdx: number, alt: any) => { 
            ...
            newItems[matchIdx] = { 
                ...
                unit_cost: alt.price * 0.6, // Reset cost on swap
                price: alt.price * oldQty, 
                image_url: alt.image_url, 
                reasoning: `VE Swap: ${alt.title}`,
                alternatives: newItems[matchIdx].alternatives // Keep alts so they can swap back
            }; 
            setItems(newItems); 
        };
            ...
            try { 
                const payload = { 
                    ...
                    items: items.map(i => ({ 
                        ...
                        // Note: We might want to save unit_cost to backend too, but keeping schema simple for now
                    })) 
                }; 
            ...
            <AuthGuard>
                <div className="min-h-screen bg-slate-50 pb-20">
                    ...
                    <div className="max-w-7xl mx-auto px-6 py-8">
                        {/* Stepper / Tabs */}
                        <div className="flex gap-8 mb-8 border-b border-slate-200 px-2">
                            <button onClick={() => setActiveTab("specs")} className={`pb-3 font-bold text-sm transition-colors flex items-center gap-2 ${activeTab === 'specs' ? 'text-brand border-b-2 border-brand' : 'text-slate-400 hover:text-slate-600'}`}>
                            ...
                            <button onClick={() => setActiveTab("pricing")} className={`pb-3 font-bold text-sm transition-colors flex items-center gap-2 ${activeTab === 'pricing' ? 'text-brand border-b-2 border-brand' : 'text-slate-400 hover:text-slate-600'}`}>
                            ...
                            <button onClick={async () => { await savePricingChanges(); setActiveTab("finalize"); }} className={`pb-3 font-bold text-sm transition-colors flex items-center gap-2 ${activeTab === 'finalize' ? 'text-brand border-b-2 border-brand' : 'text-slate-400 hover:text-slate-600'}`}>
                                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${activeTab === 'finalize' ? 'bg-brand text-white' : 'bg-slate-200 text-slate-500'}`}>3</span>
                                Review & Send
                            </button>
                        </div>
                                    ...
                                    return (
                                        <div key={i} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col lg:flex-row hover:shadow-md transition-all group">
                                            ...
                                            <div className="flex-1 p-6 bg-white relative">
                                                <div className="absolute top-0 right-0 bg-green-50 text-green-700 text-[10px] font-bold px-3 py-1 rounded-bl-xl border-l border-b border-green-100">
                                                    AI MATCH
                                                </div>
                                                {match ? (
                                                    <>
                                    ...
                                    <table className="w-full text-left">
                                        <thead className="bg-slate-50 border-b border-slate-100 text-xs font-bold text-slate-500 uppercase">
                                            <tr>
                                                ...
                                                <th className="px-4 py-3 text-right bg-red-50/50 text-red-400">Unit Cost</th>
                                                <th className="px-4 py-3 text-right bg-blue-50/50 text-blue-400">Unit Price</th>
                                                <th className="px-4 py-3 text-right">Margin</th>
                                                <th className="px-4 py-3 text-right font-extrabold text-slate-700">Total</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-slate-100 text-sm">
                                            {items.map((item, idx) => {
                                                const itemMargin = item.unit_price > 0 
                                                    ? ((item.unit_price - (item.unit_cost || 0)) / item.unit_price) * 100 
                                                    : 0;
                                                return (
                                                    <tr key={idx} className="hover:bg-slate-50">
                                                        <td className="px-4 py-3 max-w-xs">
                                                            <div className="font-bold text-slate-800 truncate">{item.product_title}</div>
                                                            <div className="text-xs text-slate-400">{item.requirement_id}</div>
                                                        </td>
                                                        <td className="px-4 py-3 text-center">
                                                            <input type="number" min="1" value={item.quantity} onChange={(e) => handleQuantityChange(idx, parseInt(e.target.value))} className="w-16 px-2 py-1 border border-slate-200 rounded text-center bg-white focus:border-brand outline-none" />
                                                        </td>
                                                        <td className="px-4 py-3 text-right bg-red-50/10">
                                                            <input 
                                                                ...
                                                                className="w-24 px-2 py-1 border border-transparent hover:border-red-200 rounded text-right bg-transparent focus:bg-white focus:border-red-400 outline-none text-red-600 font-mono" 
                                                            />
                                                        </td>
                                                        <td className="px-4 py-3 text-right bg-blue-50/10">
                                                            <input 
                                                                ...
                                                                className="w-24 px-2 py-1 border border-transparent hover:border-blue-200 rounded text-right bg-transparent focus:bg-white focus:border-blue-400 outline-none text-blue-600 font-mono font-bold" 
                                                            />
                                                        </td>
                                                        <td className="px-4 py-3 text-right">
                                                            <span className={`text-xs font-bold px-2 py-1 rounded ${itemMargin < 20 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
